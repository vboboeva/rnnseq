...
#         '''
#         # ht = h0
#         _shape = (self.d_hidden,)
#         if len(x.shape) == 3:
#             _shape = (x.shape[1], *_shape,)
#         ht = torch.randn(*_shape)
#         # hidden = ht
#         hidden = torch.Tensor([])
#         output = torch.Tensor([])
...
#             # z = self.h2h (ht + z)
#             # z = self.phi (z)
#             z = self.phi( self.h2h( ht + self.i2h( xt ) ) )
#             hidden = torch.cat([hidden, z], dim=0)
#             output = torch.cat([output, F.softmax(self.h2o(z), dim=-1)], dim=0)
#             ht = z

#         return hidden, hidden[-1], output
